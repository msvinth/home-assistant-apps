...
# HA Smart Context — generates a copilot-instructions.md with Home Assistant context
# Copilot CLI auto-loads copilot-instructions.md from ~/.copilot/, giving every session HA awareness
#
# Usage:
...
OUTPUT_FILE="${HOME}/.copilot/copilot-instructions.md"
FULL_MODE=false
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help)
            ...
            echo ""
            echo "Generate Home Assistant context for Copilot CLI sessions."
            echo "Writes a copilot-instructions.md file that Copilot automatically loads."
            echo ""
            echo "Options:"
            echo "  --full       Include entity ID listings (detailed mode)"
            echo "  --output F   Write to file F instead of \$HOME/.copilot/copilot-instructions.md"
...
> Auto-generated by Copilot Terminal app. Run \`ha-context\` to refresh.
